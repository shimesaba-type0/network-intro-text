...
`assets/images/00-*.svg`（序章のネットワーク模式図。単体PC、バス配線、集線装置＋内部トークン
リング、CSMA/CD衝突、ダムハブ接続、MACアドレステーブル、半二重/全二重比較）は、draw.ioの
GUI/CLIが使えない環境で作成したため、上記フローに従わず**SVGを直接手書きしている**。
